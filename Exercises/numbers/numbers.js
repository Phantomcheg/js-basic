...
function workWithArrays(anyArray) {
  let summary = null;
  anyArray.forEach((arrayItem) => {
    summary += arrayItem;
  });
  return parseFloat(summary).toFixed(3);
}

const arrayForCalc = [];
for (let i = 0; i <= 5; i++) {
  arrayForCalc.push(Math.random()*10);
}

console.log(workWithArrays(arrayForCalc));
